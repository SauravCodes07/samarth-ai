...
const ResultCard = ({ data, onReset, userState = "Uttar Pradesh", userDistrict = "" }) => {
  ...
  const narrative = lang === 'hi' ? (ai_advisory_text_hi || ai_advisory_text) : ai_advisory_text;
  const actionPlan = lang === 'hi' ? business_action_plan_hi : business_action_plan;
  const appSteps = lang === 'hi' ? application_steps_hi : application_steps;
    ...
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 bg-white p-4 rounded-2xl border border-slate-200 shadow-md card-hover-lift">
        ...
        <div className="flex items-center space-x-2.5">
          ...
          <button
            ...
            className={`flex items-center space-x-1.5 px-3.5 py-2 text-xs font-bold rounded-xl transition-all duration-200 ${
              isSpeaking
          ...
          >
            ...
            <span>{isSpeaking ? (lang === 'hi' ? 'आवाज रोकें' : 'Stop Audio') : (lang === 'hi' ? 'सलाह सुनें' : 'Listen Voice')}</span>
          </button>
          ...
          >
            ...
            <span>{lang === 'hi' ? 'नया फॉर्म' : 'Start Over'}</span>
          </button>

          <button
            onClick={handleDownloadPDF}
            disabled={isExporting}
            className="flex items-center space-x-1.5 px-4 py-2 text-xs font-bold text-white bg-gradient-to-r from-[#0B3D91] to-[#072a66] hover:from-[#093275] hover:to-[#041a3d] rounded-xl transition-all shadow-md disabled:opacity-50"
          >
            <FileDown className="w-4 h-4 text-[#FF9933]" />
            <span>{isExporting ? (lang === 'hi' ? 'डाउनलोड हो रहा है...' : 'Exporting...') : (lang === 'hi' ? 'PDF रिपोर्ट डाउनलोड करें' : 'Download PDF Report')}</span>
          </button>
        </div>
      </div>
      ...
      <div className="flex items-center space-x-2 bg-slate-200/70 p-1.5 rounded-2xl border border-slate-300/60 max-w-xl">
        ...
        >
          ...
          <span>{lang === 'hi' ? 'मॉड्यूल 1: व्यवहार्यता रिपोर्ट' : 'Module 1: Hyper-Local Study'}</span>
        </button>
        ...
        >
          ...
          <span>{lang === 'hi' ? 'मॉड्यूल 2: लोन संरचना व EMI' : 'Module 2: Scheme & Loan Math'}</span>
        </button>
        ...
        >
          ...
          <span>{lang === 'hi' ? 'एक्शन प्लान व रोडमैप' : 'Action Roadmap'}</span>
        </button>
      </div>
        ...
        <div className="border-b-2 border-slate-200 pb-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            ...
            <h1 className="text-2xl sm:text-3xl font-black text-[#0B3D91] tracking-tight">
              {lang === 'hi' ? (matched_scheme.scheme_name_hi || matched_scheme.scheme_name) : matched_scheme.scheme_name}
            </h1>
            <p className="text-xs sm:text-sm text-slate-600 mt-1 max-w-3xl">
              {lang === 'hi' ? matched_scheme.description_hi : matched_scheme.description}
            </p>
          </div>
          ...
          <div className="bg-gradient-to-br from-emerald-500 to-[#138808] text-white px-5 py-4 rounded-2xl text-center shadow-lg flex-shrink-0 min-w-[170px]">
            <span className="text-[10px] font-extrabold uppercase tracking-wider text-emerald-100 block">
              {loan_structure.scheme_type === 'Micro Finance Scheme' ? 'Micro Finance (<= 1.4L)' : 'Term Loan (> 1.4L)'}
            </span>
            ...
            <span className="text-[11px] font-bold bg-white/20 px-2 py-0.5 rounded-full inline-block mt-1">
              {loan_structure.tenure_years} {lang === 'hi' ? 'वर्ष पुनर्भुगतान' : 'Years Tenure'}
            </span>
          </div>
        </div>
        ...
        <div>
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-xs font-bold text-slate-500 uppercase tracking-wider flex items-center space-x-1.5">
              ...
              <span>{lang === 'hi' ? 'नियम-आधारित वित्तीय संरचना (10% मार्जिन मनी -> 90% लोन)' : 'Financial Structuring (10% Margin Money -> 90% Concessional Loan)'}</span>
            </h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="bg-slate-50 border border-slate-200 p-4 rounded-2xl card-hover-lift">
              <div className="flex items-center justify-between text-xs text-slate-600 font-semibold mb-1">
                <span>{lang === 'hi' ? 'कुल प्रोजेक्ट लागत' : 'Feasible Project Cost'}</span>
              ...
              <p className="text-[11px] text-slate-500 mt-1">
                {lang === 'hi' ? 'उपलब्ध पूंजी / 10% के आधार पर' : 'Calculated: Margin Capital / 10%'}
              </p>
            </div>
            ...
            <div className="bg-amber-50 border border-amber-300 p-4 rounded-2xl card-hover-lift">
              <div className="flex items-center justify-between text-xs text-amber-950 font-bold mb-1">
                <span>{lang === 'hi' ? '10% मार्जिन मनी (आपकी जेब से)' : '10% Margin Money (Your Share)'}</span>
              ...
              <p className="text-[11px] text-amber-800 mt-1 font-medium">
                {lang === 'hi' ? 'केवल इतना आपको स्वयं वहन करना है' : 'Upfront entrepreneur equity needed'}
              </p>
            </div>
            ...
            <div className="bg-blue-50 border border-blue-300 p-4 rounded-2xl card-hover-lift">
              <div className="flex items-center justify-between text-xs text-blue-950 font-bold mb-1">
                <span>{lang === 'hi' ? '90% सरकारी रियायती लोन' : '90% Govt Concessional Loan'}</span>
              ...
              <p className="text-[11px] text-blue-800 mt-1 font-medium">
                {lang === 'hi' ? 'राज्य एजेंसी (SCA) द्वारा स्वीकृत' : 'Funded via State Channelizing Agency'}
              </p>
            </div>
            ...
            <div className="bg-emerald-50 border border-emerald-300 p-4 rounded-2xl card-hover-lift">
              <div className="flex items-center justify-between text-xs text-emerald-950 font-bold mb-1">
                <span>{lang === 'hi' ? 'मासिक / त्रैमासिक किश्त' : 'Monthly / Quarterly EMI'}</span>
                <span className="text-[11px] text-emerald-800 font-bold bg-emerald-200/60 px-1.5 py-0.5 rounded">
                  {loan_structure.moratorium_months}m Moratorium
                </span>
              </div>
              <div className="text-2xl font-black text-[#138808]">
                ₹{loan_structure.monthly_emi.toLocaleString('en-IN')} <span className="text-xs text-emerald-700 font-normal">/mo</span>
              </div>
              <p className="text-[11px] text-emerald-800 mt-1 font-medium">
                {lang === 'hi' ? `त्रैमासिक: ₹${loan_structure.quarterly_installment.toLocaleString('en-IN')}` : `Quarterly: ₹${loan_structure.quarterly_installment.toLocaleString('en-IN')}`}
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="bg-gradient-to-br from-blue-50/80 to-indigo-50/80 p-5 rounded-2xl border border-blue-200 space-y-3">
                <div className="flex items-center space-x-2 text-[#0B3D91]">
                  ...
                  <h3 className="font-extrabold text-sm sm:text-base text-slate-900">
                    {lang === 'hi' ? '1. स्थानीय बाजार पहुंच (5-10 किमी दायरा)' : '1. Market Reach (5–10 km Radius)'}
                  </h3>
                </div>
                <p className="text-xs sm:text-sm text-slate-700 leading-relaxed font-medium">
                  {lang === 'hi' ? hyper_local_feasibility.market_reach_5_to_10km_hi : hyper_local_feasibility.market_reach_5_to_10km}
                </p>
                <div className="flex items-center space-x-2 text-xs font-bold text-blue-900 bg-white p-2.5 rounded-xl border border-blue-200">
                  ...
                  <span>{lang === 'hi' ? `अनुमानित प्रत्यक्ष उपभोक्ता आधार: ${hyper_local_feasibility.target_consumer_base_count.toLocaleString()} निवासी` : `Estimated Local Consumer Base: ~${hyper_local_feasibility.target_consumer_base_count.toLocaleString()} residents`}</span>
                </div>
                <div>
                  <span className="text-[11px] font-bold text-slate-500 uppercase tracking-wider block mb-1.5">
                    {lang === 'hi' ? 'प्राथमिक वितरण चैनल:' : 'Primary Distribution Channels:'}
                  </span>
                  <ul className="space-y-1.5 text-xs text-slate-700">
                    {(lang === 'hi' ? hyper_local_feasibility.distribution_channels_hi : hyper_local_feasibility.distribution_channels).map((ch, idx) => (
              ...
              <div className="bg-gradient-to-br from-emerald-50/80 to-teal-50/80 p-5 rounded-2xl border border-emerald-200 space-y-3">
                <div className="flex items-center space-x-2 text-emerald-950">
                  ...
                  <h3 className="font-extrabold text-sm sm:text-base text-slate-900">
                    {lang === 'hi' ? '2. अप्रयुक्त स्थानीय अवसर (Opportunity Analysis)' : '2. Opportunity Analysis (Underserved Niches)'}
                  </h3>
                </div>
                <p className="text-xs sm:text-sm text-slate-700 leading-relaxed font-medium">
                  {lang === 'hi' ? hyper_local_feasibility.opportunity_analysis_hi : hyper_local_feasibility.opportunity_analysis}
                </p>
                <div className="p-3 bg-white rounded-xl border border-emerald-200 space-y-1">
                  <span className="text-[11px] font-bold text-emerald-800 uppercase tracking-wider block">
                    {lang === 'hi' ? 'अधिशेष मांग अंतर (Unserved Demand Gap):' : 'Unserved Demand Gap:'}
                  </span>
                  <p className="text-xs text-slate-700">
                    {lang === 'hi' ? comp?.unserved_demand_gap_hi : comp?.unserved_demand_gap}
                  </p>
                </div>
              </div>
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <h3 className="font-extrabold text-sm sm:text-base text-slate-900 flex items-center space-x-2">
                    ...
                    <span>{lang === 'hi' ? '3. सामान्य व्यवसाय विश्लेषण (SWOT मेट्रिक्स)' : '3. General Business Analysis (SWOT Matrix)'}</span>
                  </h3>
                  <span className="text-[11px] text-slate-500 font-medium">
                    {lang === 'hi' ? 'प्रोजेक्ट बजट के अनुरूप तैयार' : 'Tailored to micro-enterprise budget'}
                  </span>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
                  ...
                  <div className="p-4 rounded-2xl bg-emerald-50/70 border border-emerald-300 space-y-2 card-hover-lift">
                    <div className="flex items-center space-x-1.5 text-emerald-950 font-bold text-xs uppercase tracking-wider">
                      ...
                      <span>{lang === 'hi' ? 'ताकत (Strengths)' : 'Strengths'}</span>
                    </div>
                    <ul className="space-y-1.5 text-xs text-slate-700">
                      {(lang === 'hi' ? swot.strengths_hi : swot.strengths).map((s, idx) => (
                  ...
                  <div className="p-4 rounded-2xl bg-amber-50/70 border border-amber-300 space-y-2 card-hover-lift">
                    <div className="flex items-center space-x-1.5 text-amber-950 font-bold text-xs uppercase tracking-wider">
                      ...
                      <span>{lang === 'hi' ? 'कमजोरियां (Weaknesses)' : 'Weaknesses'}</span>
                    </div>
                    <ul className="space-y-1.5 text-xs text-slate-700">
                      {(lang === 'hi' ? swot.weaknesses_hi : swot.weaknesses).map((w, idx) => (
                        <li key={idx} className="flex items-start space-x-1.5">
                  ...
                  <div className="p-4 rounded-2xl bg-blue-50/70 border border-blue-300 space-y-2 card-hover-lift">
                    ...
                    <ul className="space-y-1.5 text-xs text-slate-700">
                      {(lang === 'hi' ? swot.opportunities_hi : swot.opportunities).map((o, idx) => (
                        <li key={idx} className="flex items-start space-x-1.5">
                  ...
                  <div className="p-4 rounded-2xl bg-rose-50/70 border border-rose-300 space-y-2 card-hover-lift">
                    <div className="flex items-center space-x-1.5 text-rose-950 font-bold text-xs uppercase tracking-wider">
                      ...
                      <span>{lang === 'hi' ? 'चुनौतियां (Threats)' : 'Threats'}</span>
                    </div>
                    <ul className="space-y-1.5 text-xs text-slate-700">
                      {(lang === 'hi' ? swot.threats_hi : swot.threats).map((t, idx) => (
                        <li key={idx} className="flex items-start space-x-1.5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="p-5 rounded-2xl border border-rose-200 bg-rose-50/40 space-y-3">
                <div className="flex items-center space-x-2 text-rose-900 font-bold text-sm sm:text-base">
                  ...
                  <span>{lang === 'hi' ? '4. स्थानीय जोखिम व खतरे (Threats Identification)' : '4. Local Threats & Bottlenecks'}</span>
                </div>
                ...
                <ul className="space-y-2 text-xs text-slate-800">
                  {(lang === 'hi' ? hyper_local_feasibility.threats_identification_hi : hyper_local_feasibility.threats_identification).map((th, idx) => (
              ...
              <div className="p-5 rounded-2xl border border-slate-200 bg-slate-50 space-y-3">
                <div className="flex items-center space-x-2 text-slate-900 font-bold text-sm sm:text-base">
                  ...
                  <span>{lang === 'hi' ? '5. प्रतिस्पर्धी मैपिंग (Competitor Mapping)' : '5. Competitor Mapping & Density'}</span>
                </div>
                <div className="flex items-center justify-between bg-white p-3 rounded-xl border border-slate-200">
                  <span className="text-xs text-slate-600 font-medium">
                    {lang === 'hi' ? 'ब्लॉक में अनुमानित समान व्यवसाय:' : 'Estimated Competitors in Block:'}
                  </span>
                  <span className="text-sm font-black text-[#0B3D91] bg-blue-50 px-2.5 py-0.5 rounded-md">
                    {comp?.estimated_competitors_in_block} {lang === 'hi' ? 'इकाइयां' : 'Units'}
                  </span>
                </div>
                <div className="flex items-center justify-between bg-white p-3 rounded-xl border border-slate-200">
                  <span className="text-xs text-slate-600 font-medium">
                    {lang === 'hi' ? 'बाजार संतृप्ति स्तर (Saturation):' : 'Market Saturation:'}
                  </span>
                  <span className="text-xs font-black text-emerald-800 bg-emerald-50 px-2.5 py-0.5 rounded-md border border-emerald-200">
                    {lang === 'hi' ? comp?.saturation_level_hi : comp?.saturation_level}
                  </span>
                </div>
                <p className="text-xs text-slate-700 leading-relaxed font-medium bg-white p-3 rounded-xl border border-slate-200">
                  {lang === 'hi' ? comp?.density_analysis_hi : comp?.density_analysis}
                </p>
              </div>
            ...
            {pmv && (
              <div className="p-5 rounded-2xl bg-gradient-to-r from-amber-50/70 via-orange-50/50 to-amber-50/70 border border-amber-300 space-y-3">
                <div className="flex items-center space-x-2 text-amber-950 font-bold text-sm sm:text-base">
                  ...
                  <span>{lang === 'hi' ? '6. उत्पाद बाजार मूल्य व मूल्य निर्धारण (Product Market Value)' : '6. Product Market Value & Pricing Strategy'}</span>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  <div className="bg-white p-3.5 rounded-xl border border-amber-200">
                    <span className="text-[11px] font-bold text-amber-900 block mb-1">
                      {lang === 'hi' ? 'सुझावित मूल्य निर्धारण:' : 'Suggested Pricing:'}
                    </span>
                    <p className="text-xs font-semibold text-slate-800">
                      {lang === 'hi' ? pmv.suggested_pricing_strategy_hi : pmv.suggested_pricing_strategy}
                    </p>
                  </div>

                  <div className="bg-white p-3.5 rounded-xl border border-amber-200">
                    <span className="text-[11px] font-bold text-amber-900 block mb-1">
                      {lang === 'hi' ? 'इकाई लाभ मार्जिन (Unit Margin):' : 'Unit Margin Potential:'}
                    </span>
                    <div className="text-xl font-black text-[#138808]">
                      ~{pmv.estimated_unit_margin_percent}%
                    </div>
                    <span className="text-[10px] text-slate-500">{lang === 'hi' ? 'लागत घटाकर सकल मार्जिन' : 'Gross margin over direct costs'}</span>
                  </div>

                  <div className="bg-white p-3.5 rounded-xl border border-amber-200">
                    <span className="text-[11px] font-bold text-amber-900 block mb-1">
                      {lang === 'hi' ? 'क्षेत्रीय क्रय शक्ति संदर्भ:' : 'Purchasing Power Context:'}
                    </span>
                    <p className="text-xs text-slate-700">
                      {lang === 'hi' ? pmv.purchasing_power_context_hi : pmv.purchasing_power_context}
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200">
                <span className="text-xs text-slate-500 font-bold uppercase tracking-wider block mb-1">
                  {lang === 'hi' ? 'आवश्यक कार्यशील पूंजी (Working Capital):' : 'Working Capital Reserve:'}
                </span>
              ...
              <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200">
                <span className="text-xs text-slate-500 font-bold uppercase tracking-wider block mb-1">
                  {lang === 'hi' ? 'अनुमानित मासिक बिक्री:' : 'Est. Monthly Revenue:'}
                </span>
              ...
              <div className="bg-emerald-50 p-4 rounded-2xl border-2 border-emerald-500">
                <span className="text-xs text-emerald-900 font-bold uppercase tracking-wider block mb-1">
                  {lang === 'hi' ? 'EMI के बाद शुद्ध मासिक बचत:' : 'Net Profit After EMI:'}
                </span>
                <div className="text-2xl font-black text-[#138808]">
              ...
              <div className="p-4 bg-slate-50 border-b border-slate-200 flex items-center justify-between">
                <div className="flex items-center space-x-2">
                  ...
                  <h4 className="font-extrabold text-sm text-slate-900">
                    {lang === 'hi' ? `त्रैमासिक पुनर्भुगतान अनुसूची (${loan_structure.moratorium_months} माह मोरेटोरियम सहित)` : `Quarterly Repayment Schedule (${loan_structure.moratorium_months}-Month Moratorium Factored)`}
                  </h4>
                </div>
                <span className="text-xs bg-[#0B3D91] text-white px-2.5 py-1 rounded-full font-bold">
                  {loan_structure.tenure_years * 4} Total Quarters
                </span>
              </div>
                ...
                <table className="w-full text-left text-xs border-collapse">
                  <thead>
                    <tr className="bg-[#0B3D91] text-white">
                      ...
                      <th className="p-3 font-bold">{lang === 'hi' ? 'किश्त राशि (Installment)' : 'Installment'}</th>
                      <th className="p-3 font-bold">{lang === 'hi' ? 'मूलधन (Principal)' : 'Principal'}</th>
                      <th className="p-3 font-bold">{lang === 'hi' ? 'ब्याज (Interest)' : 'Interest'}</th>
                      <th className="p-3 font-bold rounded-r">{lang === 'hi' ? 'बकाया लोन (Balance)' : 'Closing Balance'}</th>
                    </tr>
                  </thead>
                    ...
                    {loan_structure.quarterly_schedule?.map((row) => (
                      <tr key={row.quarter} className={row.is_moratorium ? "bg-amber-50/60 font-semibold" : "hover:bg-slate-50"}>
                        <td className="p-3 font-bold text-[#0B3D91]">
                          Q{row.quarter} {row.is_moratorium && <span className="text-[10px] bg-amber-400 text-slate-950 px-1.5 py-0.5 rounded ml-1">Moratorium</span>}
                        </td>
                        <td className="p-3 text-slate-600">{row.months_label}</td>
          ...
          <div className="space-y-6 animate-fadeIn">
              ...
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center space-x-2">
                  ...
                  <h3 className="font-extrabold text-base text-slate-900">
                    {lang === 'hi' ? 'सरल भाषा में संपूर्ण वित्तीय सलाह व व्याख्या' : 'Plain Language Business & Financial Advisory'}
                  </h3>
                </div>
            ...
            <div className="p-5 rounded-2xl border border-slate-200 bg-white space-y-3">
              <h4 className="font-extrabold text-sm text-slate-900 uppercase tracking-wider">
                {lang === 'hi' ? 'व्यवसाय सफलता के लिए 4 मुख्य कदम' : '4 Key Steps for Micro-Enterprise Success'}
              </h4>
            ...
            <div className="p-5 rounded-2xl border border-emerald-200 bg-emerald-50/50 space-y-3">
              <h4 className="font-extrabold text-sm text-emerald-950 uppercase tracking-wider">
                {lang === 'hi' ? 'लोन स्वीकृति हेतु आवेदन प्रक्रिया (SCA / बैंक चरण)' : 'Application & Loan Sanction Roadmap'}
              </h4>
            ...
            <div className="bg-amber-50 border border-amber-200 p-4 rounded-2xl">
              <h5 className="font-bold text-xs text-amber-900 uppercase tracking-wider mb-1">
                {lang === 'hi' ? 'आवश्यक दस्तावेज (Required Documents):' : 'Mandatory Documents:'}
              </h5>
